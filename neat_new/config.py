...
from neat_new.six_util import iterkeys
...
class Config(object):
    """A simple container for user-configurable parameters of neat_new."""
    ...
    def __init__(self, genome_type, reproduction_type, species_set_type, stagnation_type, filename):
        ...
        # neat_new configuration
        if not parameters.has_section('neat_new'):
            raise RuntimeError("'neat_new' section not found in neat_new configuration file.")

        param_list_names = []
        for p in self.__params:
            if p.default is None:
                setattr(self, p.name, p.parse('neat_new', parameters))
            else:
                try:
                    setattr(self, p.name, p.parse('neat_new', parameters))
                except Exception:
                    ...
                    warnings.warn("Using default {!r} for '{!s}'".format(p.default, p.name),
                                  DeprecationWarning)
            param_list_names.append(p.name)
        param_dict = dict(parameters.items('neat_new'))
        unknown_list = [x for x in iterkeys(param_dict) if not x in param_list_names]
        if unknown_list:
            if len(unknown_list) > 1:
                raise UnknownConfigItemError("Unknown (section 'neat_new') configuration items:\n" +
                                             "\n\t".join(unknown_list))
            raise UnknownConfigItemError(
                "Unknown (section 'neat_new') configuration item {!s}".format(unknown_list[0]))
    ...
    def save(self, filename):
        with open(filename, 'w') as f:
            f.write('# The `neat_new` section specifies parameters particular to the neat_new algorithm\n')
