...
from neat_new.activations import ActivationFunctionSet
from neat_new.aggregations import AggregationFunctionSet
from neat_new.config import ConfigParameter, write_pretty_params
from neat_new.genes import DefaultConnectionGene, DefaultNodeGene
from neat_new.graphs import creates_cycle
from neat_new.six_util import iteritems, iterkeys


class DefaultGenomeConfig(object):
    """Sets up and holds configuration information for the DefaultGenome class."""
    allowed_connectivity = ['unconnected', 'fs_neat_new_nohidden', 'fs_neat_new', 'fs_neat_new_hidden',
...
class DefaultGenome(object):
    ...
    def configure_new(self, config):
        ...
        if 'fs_neat_new' in config.initial_connection:
            if config.initial_connection == 'fs_neat_new_nohidden':
                self.connect_fs_neat_new_nohidden(config)
            elif config.initial_connection == 'fs_neat_new_hidden':
                self.connect_fs_neat_new_hidden(config)
            else:
                if config.num_hidden > 0:
                    print(
                        "Warning: initial_connection = fs_neat_new will not connect to hidden nodes;",
                        "\tif this is desired, set initial_connection = fs_neat_new_nohidden;",
                        "\tif not, set initial_connection = fs_neat_new_hidden",
                        sep='\n', file=sys.stderr);
                self.connect_fs_neat_new_nohidden(config)
    ...
    def connect_fs_neat_new_nohidden(self, config):
        """
        Randomly connect one input to all output nodes
        (FS-neat_new without connections to hidden, if any).
        Originally connect_fs_neat_new.
        """
    ...
    def connect_fs_neat_new_hidden(self, config):
        """
        Randomly connect one input to all hidden and output nodes
        (FS-neat_new with connections to hidden, if any).
        """
        input_id = choice(config.input_keys)
